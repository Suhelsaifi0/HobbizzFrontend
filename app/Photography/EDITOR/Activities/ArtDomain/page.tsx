...
const ClubNewsForm = () => {
  ...
  const handleClickSubmitId = (id: any) => {
    router.push(`/Photography/EDITOR/Activities/ArtNews/ParticularArtNews?id=${id}`);
  };
  const gotoUpdate = (id: any) => {
    router.push(`/Photography/EDITOR/Activities/ArtDomain/Update?id=${id}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/clubdomain`, {
        method: "POST",
  ...
  const fetchNews = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/clubdomainviewallpost`);
      const data = await res.json();
